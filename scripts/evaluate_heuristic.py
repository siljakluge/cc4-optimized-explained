#!/usr/bin/env python3
"""Evaluate EnterpriseHeuristicAgent against CybORG CAGE Challenge 4.

Runs N episodes and reports mean/std total reward, comparison to dummy baseline.

Usage:
    python scripts/evaluate_heuristic.py [--episodes 20] [--steps 500] [--seed 42]
"""
...
def run_evaluation(n_episodes: int = 20, max_steps: int = 500, seed: int = 42) -> dict:
    ...
    # One agent per blue team member (use make_heuristic_agents after first reset)
    ...
    for ep in range(n_episodes):
        obs_dict, _ = env.reset()
        # Re-init agents with updated subnet hosts after each reset
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate EnterpriseHeuristicAgent on CC4")
    ...
    print(f"\nRunning {args.episodes} episodes × {args.steps} steps  (seed={args.seed})\n")
    ...
    print("\n" + "=" * 50)
    print("  EnterpriseHeuristicAgent — Evaluation Results")
    print("=" * 50)
