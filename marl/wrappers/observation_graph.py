"""
observation_graph.py — Graph Representation of Network Observations for CybORG

This module constructs and updates a dynamic heterogeneous graph of network activity observable
to blue agents in the CybORG simulation. It encodes:

- Static topology (hosts, routers, services)
- Transient connections (ports, sessions)
- Malware artifacts (files)
- Firewall policies and decoy deployments

Classes:
- NodeTracker: Maps string node IDs to numeric IDs and back
- ObservationGraph: Core class maintaining the full network observation graph

Used to produce graph-based state representations compatible with GNN architectures.
"""
...
from wrappers.nodes import SystemNode, ConnectionNode, InternetNode, FileNode, init_decoy
...
class ObservationGraph:
    ...
    def __init__(self):
        ...
        # Cached state for get_state() fast path
        self._cached_nid_map = None
        self._cached_masks = {}
        self._edges_version = 0       # bumped on any edge mutation
        self._mask_cache_version = -1  # version when masks were last cached
        self._nid_cache_version = -1   # version when nid_map was last cached

        # Pre-allocated feature buffers (reused across calls)
        self._x_buf = None
        self._tr_buf = None
        self._buf_size = 0

    def _bump_edges(self):
        '''Increment edge version counter to invalidate caches.'''
    ...
    def get_state(self, subnets):
        ...
        # Build nid_map (cache when edges haven't changed)
        ver = self._edges_version
        if self._nid_cache_version == ver and self._cached_nid_map is not None:
            nid_map = self._cached_nid_map
        else:
            nid_map = {}
            nids_list = nids.tolist()
            for i, n in enumerate(nids_list):
                nid_map[n] = i
            self._cached_nid_map = nid_map
            self._nid_cache_version = ver

        # Create mapping from old nids to reindexed nids
        nids_list_items = nids.tolist()
        nodes = [self.nodes[n] for n in nids_list_items]
        ntypes = [self.NTYPES[type(n)] for n in nodes]

        # Pre-allocate or reuse feature buffers
        dim_no_rtr = self.DIM - 9
        if n_nodes > self._buf_size:
            self._x_buf = torch.zeros(n_nodes, dim_no_rtr)
            self._tr_buf = torch.zeros(n_nodes, 9)
            self._buf_size = n_nodes
        else:
            self._x_buf[:n_nodes].zero_()
            self._tr_buf[:n_nodes].zero_()

        x = self._x_buf[:n_nodes]
        transductive_routers = self._tr_buf[:n_nodes]

        # Pre-compute router index map
        rtr_map = {r:i for i,r in enumerate(self.routers)}

        # Vectorized one-hot ntype assignment
        ntype_indices = torch.tensor(ntypes, dtype=torch.long)
        row_indices = torch.arange(n_nodes)
        x[row_indices, ntype_indices] = 1.

        # Batch subnet membership assignment
        for i, nid_val in enumerate(nids_list_items):
            name = self.nids.id_to_str(nid_val)
            sn = name[:name.index('subnet') + 6] + '_router'
            sn = self.nids[sn]
            transductive_routers[i, rtr_map[sn]] = 1

        # Batch feature extraction: group nodes by type to minimize overhead
        # Collect all features as numpy arrays, then convert once per group
        for ntype_id, offset in enumerate(self.OFFSETS):
            # Find nodes of this type that have features
            type_dim = self.NTYPE_DIMS[ntype_id]
            if type_dim == 0:
                continue
            ...
            for i in range(n_nodes):
                if ntypes[i] == ntype_id and nodes[i].dim:
                    type_indices.append(i)
                    type_features.append(nodes[i].get_features())
        ...
        # Clone to detach from buffer before cat (buffer will be reused)
        x = torch.cat([x.clone(), transductive_routers.clone()], dim=1)

        # Remap masks (cache when edges haven't changed)
        if self._mask_cache_version == ver:
            masks = [self._cached_masks[sn] for sn in subnets]
        else:
            self._cached_masks = {}
            for sn in self.subnet_masks:
                self._cached_masks[sn] = self._remap_sn_mask(sn, nid_map)
            self._mask_cache_version = ver
            masks = [self._cached_masks[sn] for sn in subnets]
    ...
    def parse_observation(self, obs):
        ...
        # Invalidate edge caches since transient edges may have changed
        self._bump_edges()
