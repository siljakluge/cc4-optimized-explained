...
from CybORG.Shared.Enums import TernaryEnum, ProcessName, ProcessType, ProcessVersion, AppProtocol, OperatingSystemType, \
...
def test_create_blank_observation(create_observation):
    ...
    assert observation.get_dict()['success'] == TernaryEnum.UNKNOWN
    assert type(observation.get_dict()['success']) == TernaryEnum
...
def test_set_success_true(set_success_true):
    ...
    assert observation.get_dict()['success'] == TernaryEnum.TRUE
    assert type(observation.get_dict()['success']) == TernaryEnum
    assert observation
...
def test_set_success_false(set_success_false):
    ...
    assert observation.get_dict()['success'] == TernaryEnum.FALSE
    assert type(observation.get_dict()['success']) == TernaryEnum
...
def test_add_info_process_3(add_info_process_3):
    ...
    assert observation.get_dict()["test"]["Processes"][0]["PID"] == 1
    assert observation.get_dict()["test"]["Processes"][0]["PPID"] == 2
    assert observation.get_dict()["test"]["Processes"][0]["process_name"] == "hello"
...
def test_add_info_process_4(add_info_process_4):
    ...
    assert observation.get_dict()["test"]["Processes"][0]["process_name"] == "hello"
    assert observation.get_dict()["test"]["Processes"][0]["Known Process"] == ProcessName.UNKNOWN
    assert observation.get_dict()["test"]["Processes"][0]["process_type"] == ProcessType.SSH
...
def test_all_process_info(add_all_process_info):
    ...
    assert observation.get_dict()["test"]["Processes"][0]["process_name"] == "svchost"
    ...
    assert observation.get_dict()["test"]["Processes"][0]["process_type"] == ProcessType.UNKNOWN
...
def test_add_interface_info(add_interface_info):
    ...
    assert interface["interface_name"] == "test_interface"
    assert interface["ip_address"] == IPv4Address("10.11.12.13")
...
def test_add_user_info(add_user_info):
    ...
    assert user["password_hash"] == "5f4dcc3b5aa765d61d8327deb882cf99"
    assert user["password_hash_type"] == PasswordHashType.MD5
